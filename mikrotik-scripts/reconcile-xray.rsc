...
:local proxyBase "https://proxy.domain.tld"
...
:if ([:len $idMain] > 0) do={
    ...
    :if ($hostMain = "") do={
        :set hostMain [/tool netwatch get $idMain host]
    }
}
...
:if ([:len $idB1] > 0) do={
    ...
    :if ($hostB1 = "") do={
        :set hostB1 [/tool netwatch get $idB1 host]
    }
}
...
:if ([:len $idB2] > 0) do={
    ...
    :if ($hostB2 = "") do={
        :set hostB2 [/tool netwatch get $idB2 host]
    }
}
...
:if ([:len $idB3] > 0) do={
    ...
    :if ($hostB3 = "") do={
        :set hostB3 [/tool netwatch get $idB3 host]
    }
}
...
:if (($mainLocal = 0) and ($mainRemote > 0)) do={
    :set stMain "LOCAL_DOWN"
}
:if (($mainLocal > 0) and ($mainRemote = 0)) do={
    :set stMain "REMOTE_DOWN"
}
...
:if ([/ip route print count-only where routing-table="r_to_vpn" dst-address="0.0.0.0/0" gateway="172.18.20.6" active] > 0) do={
    :set current "main"
}
:if ([/ip route print count-only where routing-table="r_to_vpn" dst-address="0.0.0.0/0" gateway="172.18.21.6" active] > 0) do={
    :set current "backup1"
}
:if ([/ip route print count-only where routing-table="r_to_vpn" dst-address="0.0.0.0/0" gateway="172.18.22.6" active] > 0) do={
    :set current "backup2"
}
:if ([/ip route print count-only where routing-table="r_to_vpn" dst-address="0.0.0.0/0" gateway="172.18.23.6" active] > 0) do={
    :set current "backup3"
}
...
:if ($text != "") do={
    ...
    :local url ($proxyBase . "/bot/bot" . $token . "/sendMessage?chat_id=" . $chat . "&message_thread_id=" . $thread . "&text=" . $report)

    :do {
        :local r [/tool fetch url=$url http-percent-encoding=yes output=user as-value check-certificate=no]
        :log info ("TG sent: " . $text . " status=" . ($r->"status"))
    } on-error={
        :log error ("TG send failed: " . $text)
    }
}
